    ...
    const articles = await page.evaluate(() => {
      ...
      items.forEach((item, index) => {
        if (index < 5) { // Limit to the first 5 articles
          ...
          let imageUrl = item.querySelector('article > div > div > div > div > div:nth-child(1) > div > div:nth-child(2) > div:nth-child(2) > div > div > img')?.getAttribute('src') || '';
          ...
          results.push({ title, author, authorProfile, likeCount, commentCount, publicationDate, url, imageUrl });
        }
      });
      return results;
    });
